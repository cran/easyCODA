...
PLOT.LRA <- function(obj, map="symmetric", rescale=1, dim=c(1,2), main="", cex=c(0.8,0.8), col=c("blue","red"), colarrows = "pink", axes.inv = c(1,1)) {
# plotting function for LRA objects
# map       = "symmetric" / "asymmetric" / "contribution"
# rescale   = rescaling of column coordinates
# cex       = c(0.8,0.8) (default) character explansion factors for row and column labels
# col       = c("blue","red") (default) colours for row and column labels
...
# axes.inv  = c(1,1) (default); e.g. = c(1,-1) for inverting second axis, or c(-1,-1) for both
  ...
  if(rescale !=1) {
    ...
    axis(1)
    axis(2)
    axis(3, at=axTicks(3), labels=round(axTicks(3)/rescale,2), col="black", col.ticks=col[2], col.axis=col[2])
    axis(4, at=axTicks(4), labels=round(axTicks(4)/rescale,2), col="black", col.ticks=col[2], col.axis=col[2])
  }
  ...
  text(rescale*obj.crd, labels=obj$colnames, col=col[2], cex=cex[1], font=4)
}

PLOT.PCA <- function(obj, map="symmetric", rescale=1, dim=c(1,2),
                     axes.inv=c(1,1), main="",
...
# fonts     = c(2,4) (default) font choice for rows and column labels)
  ...
  if(rescale !=1) {
    ...
    axis(1)
    axis(2)
    axis(3, at=axTicks(3), labels=round(axTicks(3)/rescale, 1), col=cols[2], col.ticks=cols[2], col.axis=cols[2])
    axis(4, at=axTicks(4), labels=round(axTicks(4)/rescale, 1), col=cols[2], col.ticks=cols[2], col.axis=cols[2])
  }
